...
**Scope decision (2026-07-13): grandparent-focused, association-agnostic.** V1 positioning, prompt packs, and marketing target the grandparent↔grandchild relationship — but nothing in the model may preclude other associations (parent, great-aunt, family friend, mentor, veteran neighbor, church elder). Concretely: "storyteller" and "asker" are *per-story roles*, not member types; kinship is never encoded in the schema; the storyteller is a `Person` in the space who may or may not be linked to an account (see elder-no-device below); prompt packs carry the association flavor, so a "veterans" or "mentor" pack later is content, not code. This also widens the org-pack story — a church buying passes for a grandparents' ministry and a hospice buying them for life-review sessions run on the identical product.

**Scope decision (2026-07-13): the elder needs no device in v1.** Recording happens in person on the grandkid's or parent's phone; the grandparent is the storyteller, not a required app user. This dissolves the elder-onboarding/invite/daily-question UX risk from v1 entirely and shrinks scope. Elder-side features (own device, answering queued questions remotely, daily prompts) move to the remote-mode roadmap. Note: no-device ≠ no elder UX — the in-session screen is still elder-visible (question text readable across a table, unambiguous recording state), so the session view is designed for the storyteller's eyes even on someone else's phone.
...
**Kill criteria:**
1. **CloudKit sharing spike fails** — if the shared-zone model can't pass the spike scope below, and the fallback requires running my own backend, the no-server privacy moat collapses; re-evaluate before writing feature code. Spike must verify: (a) shared-zone `CKAsset` audio at realistic sizes syncing to all participants; (b) invite/accept flow across different Apple IDs in different Apple Family groups — v1 invitees are adult family members across households (per the elder-no-device scope decision), so the bar is "smooth for a busy parent"; elder-tolerable joins become a remote-mode requirement; (c) storage accounting — confirm assets in the shared zone count against the *zone owner's* iCloud quota and measure real consumption per story; (d) an entitlement record written by the owner propagating to participants promptly enough to gate features; (e) behavior when a participant has no Apple device — what a web/read-only fallback would require.
...
- **End-goal artifact (roadmap north star, 2026-07-13):** someone in the family — a curator role, likely but not necessarily the host — takes the raw recordings, curates them, does basic audio/video editing, and assembles a composed **"memory archive"**: "Grandpa's Navy Years," "Grandma's Kitchen," a milestone-birthday or memorial compilation. This is the completion payoff the review mining says families treasure (StoryWorth's book, Linda's podcast — the artifact that rewards finishing), and it monetizes completion rather than anxiety. Not v1 — but **v1 must not foreclose it**, which imposes two architecture commitments now: (1) **originals are immutable** — all editing is non-destructive metadata (clip in/out points, ordering, chapters) referencing untouched source audio; a future `Compilation` → `CompilationItem(storyID-as-plain-UUID, in/out, order)` fits the single-FK tree rule cleanly; (2) **transcripts carry timestamps** — timed transcripts (SpeechAnalyzer provides them) make *text-based* curation possible later — select the sentences you want, get the audio clip — which is the only editing paradigm a non-editor family member will actually use. The canonical export format must preserve both.

  **The artifact ladder (roadmap beyond the in-app compilation):** the composed archive should eventually leave the app as real artifacts, in stages of increasing fidelity — and the governing principle is that **Passalong produces structured intermediates; it never becomes a book-design or video-editing tool**:
  1. **Self-contained shareable/printable artifact first — no hosting, ever (decided 2026-07-13).** A curated collection rendered as a file the family passes around directly: a well-designed **PDF** (printable at home, sendable to any print shop — Lulu-style services accept PDF directly, so the simple path to "a real book" may need no intermediate tool at all) and/or a **single-file HTML** bundle with embedded photos and audio for AirDrop/Messages/email. Passalong never hosts anything; sharing is the family handing a file to family.
  2. **Book-production handoff** — export structured content (ordered chapters, transcript text, images, captions) in a form a real book tool consumes, feeding a Lulu-style print-on-demand product. Passalong's job ends at a clean manuscript-intermediate; layout craft belongs to tools built for it.
  3. **AV-editing handoff** — same pattern for audio/video: export the curation as something iMovie-class tools can use as a starting point — trimmed, named, ordered clips per the compilation metadata (chapter lists alongside; timeline-interchange formats like FCPXML as a stretch for pro tools). The non-destructive `CompilationItem` metadata is exactly the data such an export renders.

  All three rungs are downstream consumers of the same canonical export (immutable originals + timed transcripts + curation metadata) — more weight on that format, still no new v1 scope beyond getting it right.
- **Out of scope for v1:** voice cloning of the deceased (Forevermore's Echo — ethically loaded, not this product), Android, print books (partner later), AI interviewer, curation/editing tools (see end-goal artifact above — enabled by v1's data decisions, built later).
...
- **Keepsake add-ons** — print-on-demand book per completed collection, archival USB/audio-CD for elders, and (roadmap) the composed **memory-archive compilation** — curated, lightly edited audio/video assembled from the raw stories (see the end-goal artifact in Concept). One-time purchases that monetize completion instead of anxiety. Likely the second revenue line, competing for StoryWorth's book dollar with a richer artifact than print.
...
- Artifact ladder (roadmap): what intermediate formats do book tools and iMovie-class editors actually accept well (manuscript structure; clip bundles vs. FCPXML)? And does a well-made PDF cover the print path end-to-end (Lulu-style services take PDF directly), making the book-tool handoff rung optional? Research when the compilation feature approaches. (Hosting is decided: never — artifacts are self-contained files the family shares directly.)

## Related

- [[UX Flows]] — v1 journey map (four actors × seven phases), session/queue/recording flows, session-screen principles, open UX questions (draft)
