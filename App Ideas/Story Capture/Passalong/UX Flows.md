...
| **Visit — during** | Talks. Sees across the table: the current question in large type + an unambiguous recording indicator. Never asked to touch anything. | Asks the question out loud; taps once when Grandma finishes ("next"); can toss in a spontaneous follow-up. | Mostly stays out of it. Handles the rare mechanical need (retake, pause for lunch). | — |
...
1. **Readable at four feet** — the current question in type sized for a grandmother across a kitchen table, not a user holding a phone.
