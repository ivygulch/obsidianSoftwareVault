...
| **Between visits** | Nothing. (Remote-mode roadmap: daily question on their own device.) | Adds questions to the queue — own words or picked from packs ("Ask Grandpa about…"). | Nudges kids to queue questions; peeks at queue before the visit ("we have 7 for Grandma"). | Adds questions from afar; sees queue grow; anticipation is the engagement loop. |
...
1. **The human asker is the storyteller's interface — the screen is the operator's.** In video arrangements (tripod, handheld) the camera faces the storyteller and the screen faces away, so the storyteller likely never sees it: the question always travels *by voice*, from the grandkid, and the screen serves whoever is behind the device (question to read aloud, rec state, timecode). Only in the audio-on-the-table arrangement is the screen visible to everyone — there, the current question in type readable at four feet is a nicety worth having. A storyteller-facing *display* (large question, photo prompts) is what a second device adds — never something v1's single device promises.
